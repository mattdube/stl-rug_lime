...
nnet_cm <- confusionMatrix(nnnet.predict, test_x$Churn, positive = "Yes")
nnet_accuracy <- c(nnet_cm$overall[c(1,3,4)],nnet_cm$byClass[7])

mars_cm<- confusionMatrix(mars.predict, test_x$Churn, positive = "Yes")
mars_accuracy <- c(mars_cm$overall[c(1,3,4)],mars_cm$byClass[7])

xgb_cm <- confusionMatrix(xgb.predict, test_x$Churn, positive = "Yes")
